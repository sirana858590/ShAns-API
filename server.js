...
// Track sent items to avoid repeats
let sentVideos = [];
let sentPhotos = [];

// Girls Video Endpoint
app.get('/ShAn/girlsvideo', (req, res) => {
  try {
    if (sentVideos.length >= girlsVideos.length) {
      sentVideos = [];
    }
...
app.get('/ShAn/dpboy', (req, res) => {
  try {
    if (sentPhotos.length >= boyPhotos.length) {
      sentPhotos = [];
    }

    const availablePhotos = boyPhotos.filter(photo => !sentPhotos.includes(photo));
    const randomPhoto = availablePhotos[Math.floor(Math.random() * availablePhotos.length)];
    
    sentPhotos.push(randomPhoto);
    res.json({ url: randomPhoto });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
